...
function createDatabase() { 
    conn.query('CREATE TABLE cases (patient_uuid SERIAL PRIMARY KEY, name VARCHAR(50), health_insurance_no LONG, severity INTEGER, condition VARCHAR(2000), eta DATE);',
        function (err, results, fields) {
            if (err) throw err;
            console.log('Create cases table.');
        })
    
    conn.query('INSERT INTO cases (name, health_insurance_no, severity, condition, eta) VALUES (?, ?, ?, ?, ?);',
...
function insertData(){
    conn.query('INSERT INTO cases (name, health_insurance_no, severity, condition, eta) VALUES (?, ?, ?, ?, ?);', 
        [req.name, req.health_insurance_no, req.severity, req.condition, req.eta],
        function (err, results, fields) {
            if (err) throw err;
            else console.log('Inserted ' + results.affectedRows + ' rows(s).');
        })
